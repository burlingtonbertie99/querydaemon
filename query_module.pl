...
%?- load_todoist_sync_file('sync-response.json', Term).
% Term = todoist_sync(sync_token("..."), full_sync(true),
% sync_status([]), ... ).

%?- load_todoist_sync_file('sync-response.json', Term), validate_todoist_sync(Term, Result).
%Result = ok.  % or a list of errors
